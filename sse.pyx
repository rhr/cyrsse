...
cdef int bisse(double t, double y[], double f[], void *params) nogil:
    ...
    # rate of change in each state's frequency
    f[0] = -(lambda0+mu0+q01)*D0 + q01*D1 + 2*lambda0*E0*D0   # dD0/dt
    f[1] = -(lambda1+mu1+q10)*D1 + q10*D0 + 2*lambda1*E1*D1   # dD1/dt
    f[2] = mu0-(mu0+q01+lambda0)*E0 + q01*E1 + lambda0*E0**2  # dE0/dt
    f[3] = mu1-(mu1+q10+lambda1)*E1 + q10*E0 + lambda1*E1**2  # dE1/dt
    return GSL_SUCCESS
